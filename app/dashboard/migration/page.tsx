...
import {
  ArrowLeft, ArrowRight, Check, Upload, Folder, FileText,
  Image, Box, Volume2, ChevronRight, Search, X, Loader2,
...
type Source = "google_drive" | "dropbox" | null;
type Step = 1 | 2 | 3;
...
function fileIcon(name: string, isFolder: boolean) {
...
function isAssetFile(name: string) {
  const ext = name.split(".").pop()?.toLowerCase() || "";
  const supported = [
    "png", "jpg", "jpeg", "gif", "svg", "webp", "tga", "psd", "exr", "hdr",
    "fbx", "obj", "gltf", "glb", "usd", "usda", "usdz", "blend", "ma", "mb", "max",
    "wav", "mp3", "ogg", "flac", "aiff",
    "unity", "prefab", "asset", "mat", "shader", "anim", "controller",
    "cs", "hlsl", "cginc", "compute",
    "json", "xml", "yaml", "txt",
  ];
...
export default function MigrationPage() {
  ...
  // ── Init ──
  useEffect(() => {
    const t = localStorage.getItem("kuantum_token") || "";
    const s = localStorage.getItem("kuantum_studio") || "";
    const studio = JSON.parse(localStorage.getItem("lead_studio") || "{}");
    setToken(t);
    setStudioSlug(s);
    setStudioName(studio?.name || "Studio");
    if (t) {
      api.fetchFolders(t).then(setFolders).catch(() => {});
    }
  }, []);
  ...
  const loadFiles = useCallback(async (folderId?: string) => {
    ...
    try {
      ...
      if (source === "google_drive") {
        ...
        const path = folderId || "";
        result = await api.listDropboxFiles(token, oauthToken, path);
      }
      setFiles(result);
    } catch {
      setFiles([]);
  ...
  }, [token, oauthToken, source]);

  const navigateToFolder = useCallback((file: CloudFile) => {
  ...
  const selectAll = useCallback(() => {
    const selectable = files.filter(f => !f.isFolder && isAssetFile(f.name));
    if (selectable.every(f => selectedIds.has(f.id))) {
      setSelectedIds(prev => {
        const next = new Set(prev);
        selectable.forEach(f => next.delete(f.id));
        return next;
      });
    } else {
      setSelectedIds(prev => {
        const next = new Set(prev);
        selectable.forEach(f => next.add(f.id));
        return next;
      });
    }
  }, [files, selectedIds]);

  // ── Connect OAuth (simulated with token input for now) ──
  ...
  const startImport = useCallback(async () => {
    ...
    try {
      ...
      if (result.error) {
        setImportResults([{ file_id: "", name: "Import", status: "failed", error: result.error }]);
      } else {
        setImportResults(result.results || []);
      }
    } catch (e: any) {
      setImportResults([{ file_id: "", name: "Import", status: "failed", error: e.message }]);
    }
    setImporting(false);
    setImportDone(true);
  }, [token, source, oauthToken, selectedIds, targetFolderId]);
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-[#EDEDED]">
      <DashboardNav studioName={studioName} role="lead" isEnterprise={true} showSearch={false} />
      ...
      <div className="border-b border-[#1E1E1E] bg-[#0A0A0A]/50">
        <div className="max-w-5xl mx-auto px-6 py-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-[#52525B]">Import assets from cloud storage into your workspace</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            {[
              ...
              <div key={n} className="flex items-center gap-2">
                {i > 0 && <div className={`w-8 h-px ${step >= n ? "bg-blue-500" : "bg-[#27272A]"}`} />}
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`text-[10px] font-medium ${step >= n ? "text-[#A1A1AA]" : "text-[#3F3F46]"}`}>
                    {label}
                  </span>
                </div>
              </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              ...
              <p className="text-xs text-[#52525B] mb-8">Select where your assets are stored. We'll connect securely and let you browse files.</p>
                  ...
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl bg-[#0A0A0A] border border-[#27272A] flex items-center justify-center">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
                        ...
                        <path d="M12.012 15.616l-5.008 3.973-2.004-1.308v1.464l7.012 4.21 7.012-4.21v-1.464l-2.004 1.308-5.008-3.973z" fill="#0061FF"/>
                      </svg>
                    </div>
                    <div>
                  ...
                  <div className="rounded-xl border border-[#1E1E1E] bg-[#121212] p-5 mb-6">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 rounded-lg bg-[#0A0A0A] border border-[#27272A] text-xs text-[#EDEDED] placeholder:text-[#3F3F46] focus:border-[#3F3F46] focus:ring-0 focus:outline-none"
                      />
                      <button
                        onClick={connectSource}
                        disabled={!oauthToken}
                        className="px-4 py-2 rounded-lg bg-blue-600 text-xs font-medium text-white hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                      >
                        Connect & Browse
                      </button>
                    </div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs text-[#52525B]">
                    Browse your {source === "google_drive" ? "Google Drive" : "Dropbox"} and select files to import.
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => { setStep(1); setFiles([]); setBreadcrumb([]); }}
                    className="px-3 py-1.5 rounded-lg border border-[#27272A] text-xs text-[#71717A] hover:text-white hover:border-[#3F3F46] transition-colors"
                  >
                    Change Source
                  </button>
                </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-3 py-2 rounded-lg bg-[#0A0A0A] border border-[#27272A] text-xs text-[#EDEDED] placeholder:text-[#3F3F46] focus:border-[#3F3F46] focus:ring-0 focus:outline-none"
                  />
                  {searchQuery && (
                ...
                <button
                  onClick={selectAll}
                  className="px-3 py-2 rounded-lg border border-[#27272A] text-[10px] text-[#71717A] hover:text-white hover:border-[#3F3F46] transition-colors whitespace-nowrap"
                >
                  Select All Assets
                </button>
                <button
                  onClick={() => loadFiles(breadcrumb.length > 0 ? breadcrumb[breadcrumb.length - 1].id : undefined)}
                  className="px-3 py-2 rounded-lg border border-[#27272A] text-[10px] text-[#71717A] hover:text-white hover:border-[#3F3F46] transition-colors"
                >
                  Refresh
                </button>
              </div>
              ...
              <div className="rounded-xl border border-[#1E1E1E] bg-[#121212] overflow-hidden mb-6">
                ...
                <div className="grid grid-cols-[40px_1fr_120px_120px] gap-2 px-4 py-2.5 border-b border-[#1E1E1E] bg-[#0A0A0A]">
                  <span className="text-[9px] text-[#52525B] font-medium uppercase"></span>
                  <span className="text-[9px] text-[#52525B] font-medium uppercase tracking-wider">Name</span>
                  <span className="text-[9px] text-[#52525B] font-medium uppercase tracking-wider text-right">Size</span>
                  <span className="text-[9px] text-[#52525B] font-medium uppercase tracking-wider text-right">Modified</span>
                </div>
                      ...
                      return (
                        <div
                          ...
                          onClick={() => {
                            ...
                            else if (selectable) toggleSelection(file.id);
                          }}
                          className={`grid grid-cols-[40px_1fr_120px_120px] gap-2 px-4 py-2.5 border-b border-[#1E1E1E]/50 cursor-pointer transition-colors ${
                            selected
                              ? "bg-blue-500/5"
                              : file.isFolder
                              ? "hover:bg-white/[0.02]"
              ...
              <div className="flex items-center justify-between">
                <button
                  onClick={() => setStep(1)}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-[#27272A] text-xs text-[#71717A] hover:text-white hover:border-[#3F3F46] transition-colors"
                >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-5 py-2 rounded-lg bg-blue-600 text-xs font-medium text-white hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
              ...
              {!importDone ? (
                <>
                  ...
                  <p className="text-xs text-[#52525B] mb-8">
                    Confirm your selection and choose a target folder. Files will be downloaded and uploaded to your workspace.
                  </p>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6">
                    <div className="rounded-xl border border-[#1E1E1E] bg-[#121212] p-4">
                      ...
                      <p className="text-sm font-semibold">{source === "google_drive" ? "Google Drive" : "Dropbox"}</p>
                    </div>
                    <div className="rounded-xl border border-[#1E1E1E] bg-[#121212] p-4">
                  ...
                  <div className="rounded-xl border border-[#1E1E1E] bg-[#121212] p-5 mb-6">
                    ...
                    <p className="text-[10px] text-[#52525B] mb-3">Choose where to store the imported assets in your workspace.</p>
                    <select
                      value={targetFolderId}
                      onChange={(e) => setTargetFolderId(e.target.value)}
                      className="w-full px-3 py-2 rounded-lg bg-[#0A0A0A] border border-[#27272A] text-xs text-[#EDEDED] focus:border-[#3F3F46] focus:ring-0 focus:outline-none appearance-none"
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-gradient-to-r from-blue-600 to-purple-600 text-xs font-semibold text-white hover:from-blue-500 hover:to-purple-500 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-lg shadow-blue-500/20"
                    >
                      {importing ? (
                        <>
                ...
                <>
                    ...
                    <div className="max-h-[300px] overflow-y-auto">
                      {importResults.map((result, i) => (
                        ...
                        >
                          ...
                          {result.error && (
                            <span className="text-[10px] text-rose-400/70 max-w-[200px] truncate">{result.error}</span>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center justify-center gap-3">
                    <button
                      ...
                      className="px-4 py-2 rounded-lg border border-[#27272A] text-xs text-[#71717A] hover:text-white hover:border-[#3F3F46] transition-colors"
                    >
                      Import More
                    </button>
                    <a
