...
import { Layers, ArrowRight, Loader2, Shield, KeyRound } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#09090B] flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative w-full max-w-[400px] px-6">
        ...
        <div className="rounded-2xl border border-[#1E1E1E] bg-[#121212]/80 backdrop-blur-sm p-7 shadow-2xl shadow-black/30">
          <h1 className="text-lg font-semibold text-[#EDEDED] mb-1 tracking-tight">Welcome back</h1>
          <p className="text-xs text-[#52525B] mb-6">Sign in to your workspace</p>

          {/* Mode toggle */}
          <div className="mb-6 grid grid-cols-2 gap-1 rounded-lg bg-[#0A0A0A] border border-[#1E1E1E] p-1">
            <button
          ...
          <form onSubmit={mode === "lead" ? handleLogin : handleMemberLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-10 flex items-center justify-center gap-2 rounded-lg bg-white text-[#09090B] text-sm font-semibold hover:bg-white/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg shadow-white/5"
            >
              {loading ? (
